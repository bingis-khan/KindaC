{-# LANGUAGE LambdaCase, DeriveTraversable, GeneralisedNewtypeDeriving, TemplateHaskell, TypeFamilies, UndecidableInstances #-}
...
-- Function --
--------------
type family FunDef phase
data GFunDef g l t stmt = FD g [(l, t)] t (NonEmpty stmt) deriving (Show, Functor)

-- We only take names into account when seearching for a function
-- so instances should reflect this.
instance Eq g => Eq (GFunDef g l tid stmt) where
  FD name _ _ _ == FD name' _ _ _ = name == name'

instance Ord g => Ord (GFunDef g l tid stmt) where
...
data TopLevel
  = FunDef (FunDef Untyped)
  | DataDef (DataDef Untyped)
  | TLStmt (Stmt Untyped)
  deriving Show
...
type instance Stmt Untyped = Fix (StmtF Text Text (Expr Untyped))
type instance DataCon Untyped = GDataCon Text (Type Untyped)
type instance DataDef Untyped = GDataDef Text Text (DataCon Untyped)
type instance FunDef Untyped = GFunDef Text Text (Maybe (Type Untyped)) (Stmt Untyped)
...
type instance Stmt Resolved = Fix (StmtF Local Global (Expr Resolved))
...
type instance Stmt Typed = Fix (StmtF Local Global (Expr Typed))
...
$(deriveBifoldable ''GFunDef)
$(deriveBifunctor ''GFunDef)
$(deriveBitraversable ''GFunDef)
