...
cModule MonoModule{ functions, dataTypes, main } = pp $ do
  comment "DATATYPES"
...
cFunction :: Annotated (FunDec Mono, NonEmpty (AnnStmt Mono)) -> PP
cFunction = undefined

cDataType :: Annotated (DataDef Mono) -> PP
cDataType = undefined

cMain :: [AnnStmt Mono] -> PP
cMain = ccFunction "main" "void" [] . fmap cStmt

cStmt :: AnnStmt Mono -> PP
cStmt = cata $ \(AnnStmt anns monoStmt) -> case monoStmt of
  EnvStmt me monoenv -> statement $ do
    cMonoEnvType monoenv
    cMonoEnvInfo me
    "="
    cMonoEnv monoenv

  NormalMonoStmt stmt -> case stmt of
...
cType :: Type Mono -> PP
cType = cata $ \case
  MTCon mti -> cMonoTypeInfo mti  -- the type should be defined by that point.
  MTFun env args ret -> typedef env args ret
